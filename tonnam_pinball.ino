...
void checkScoreSensor() {
  ...
  if (millis() - lastDebugTime > 1000) {  // Every 5 seconds
...
void checkSongSensors() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    static unsigned long lastDebugTime = 0;
    if (millis() - lastDebugTime > 1000) {  // Every 5 seconds
      Serial.print("Song sensor value: ");
      Serial.println(sensorValue);
      lastDebugTime = millis();
    }
    ...
    if (currentState && !prevSongSensorStates[i]) {
      ...
      // If song 0x03 (003.wav) is played, increment the score
      if (UNIQUE_SONGS[i] == 0x03) {
        // Only increment if we're not in an animation and with debounce
        if (!ledAnimationActive && millis() - lastScoreTime > DETECTION_DEBOUNCE) {
          scoreCount++;
          lastScoreTime = millis();
          ...
          // Debug print
          Serial.print("Score increased from song 003: ");
          Serial.println(scoreCount);

          // Check if we've reached 5 or 10 detections after the increment
          if (scoreCount == 5) {
            // Play song 001.wav from folder 01 at the first 5 detections
            mp3.playWithFileName(FOLDER_NUMBER, SCORE_SONG[0]);
            Serial.println("Playing score song 1 (001.wav)");
...
void checkMotorLightsSensor() {
  ...
  static unsigned long lastDebugTime = 0;
  if (millis() - lastDebugTime > 1000) {  // Every 5 seconds
    Serial.print("Motor Lights sensor value: ");
    Serial.println(sensorValue);
    lastDebugTime = millis();
  }
